        ...
        for (what, prop, uri) in self.forward_refs:
            if uri in self.uri_object_map:
                ...
                raise NotImplementedError(
                    "No class information for %s.%s = %s".format(what, prop, uri))
    ...
    def construct(self, js):
        ...
        if typ == None:
            ...
            if set(typ) == set(["EndOfExistence", "Activity"]):
                ...
                raise NotImplementedError(
                    "Not sure which of multiple classes to use!")
        else:
            ...
            if typ == "Payment":
                ...
                try:
                    ...
                    # No such class
                    raise DataError(
                        "Resource %s has unknown class %s" % (ident, typ))
        ...
        for (prop, value) in itms:
            ...
            for subvalue in value:
                if rng == str:
                    ...
                    # No idea!!
                    raise DataError(
                        "Value %r is not expected for %s" % (subvalue, prop))
