...
# gvp - 1. win path fix  2. 'file' is deprecated, used 'open'
...
# gvp - 1. win path fix (curr_dir defined earlier)  2. 'file' is deprecated, used 'open'
rel_path = 'data'
...
def process_classes(dom):
    ...
    for c in classes:
        ...
        # gvp - has_key() is depricated. Used 'in' instead
        ...
        label = c.xpath(
            './rdfs:label[@xml:lang="en"]/text()', namespaces=NS)[0]
        try:
...
def process_props(dom):
    ...
    for p in props:
        ...
        if not inverse:
            inverse = invdom.xpath(
                '//rdf:Property[@rdf:about="%s"]/owl:inverseOf/@rdf:resource' % name, namespaces=NS)
        if inverse:
            inverse = inverse[0]
        else:
            inverse = ""
        ...
        ##
            ...
            # gvp - has_key() is deprecated. Used 'in' instead
...
for fn in files:
    ...
    # gvp - win path fix ('rel_path' defined earlier)
...
# gvp - win path fix ('curr_dir' defined earlier)
data_dir = os.path.abspath(os.path.join(curr_dir, '..', 'cromulent/data'))
