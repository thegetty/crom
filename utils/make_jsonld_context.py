...
for l in lines:
    ...
    if info[1] == "class":
        ...
        # gvp - 'has_key' deprecated, used 'in' instead
        # if which.has_key(ctname):
        ##
        if ctname in which:
            print("Already found: %s   (%s vs %s)" %
                  (ctname, context[ctname]['@id'], name))
        else:
            if rng:
                if rng[0] == "E":
                    typ = "@id"
...
# gvp - 1. win path fix ('data_dir' defined earlier) 2. 'file' is deprecated in Python 3
